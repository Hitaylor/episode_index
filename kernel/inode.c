...
static int episode_fill_super(struct super_block *s, void *data, int silent)
{
	...
	struct episode_super_block *ms = NULL;
	...
	BUILD_BUG_ON(128 != sizeof(struct episode_inode));//jin
	...
	if ( *(__u16 *)(bh->b_data + 24) == EPISODE_SUPER_MAGIC) {
		ms = (struct episode_super_block *) bh->b_data;
		sbi->s_ms = ms;
	  sbi->s_sbh = bh;
...
static int episode_get_block(struct inode *inode, sector_t block,
...
static void episode_write_failed(struct address_space *mapping, loff_t to)
{
