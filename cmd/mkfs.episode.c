...
static char *inode_buffer = NULL;//这个就是指向inode表的指针
#define Inode (((struct episode_inode *) inode_buffer) - 1) //(struct episode_inode *) inode_buffer是一个指针，长度为4字节，指向episode_inode的某个对象a，也就是inode_buffer的值为这个对象a的地址;指针-1是表示一个指针，指向a之前的那个对象，也就是指针-1之后的值为a的地址减去sizeof(episode_inode);
//这种写法的目的是解决inode号从1开始计数，而数组从0开始计数;inode为5的节点存储在inode表的第4个位置（0为计数起点）。这么做的目的是防止inode_buffer[0]浪费
...
static void make_root_inode_episode(void) {
	...
	if (inode->i_uid)
		inode->i_gid = getgid();
	inode->i_indexcount = 0;//jin
	memset(inode->i_index,0,sizeof(inode->i_index));//jin	
...
static void setup_tables(void) {
	...
	Super.s_blocksize = EPISODE_BLOCK_SIZE;//jin
	...
	if (inodes > EPISODE_MAX_INODES)
		inodes = EPISODE_MAX_INODES;
...
int main(int argc, char ** argv) {
	...
	*(int *)tmp = 1;
	...
	if (BLOCKS > EPISODE_MAX_INODES)
		BLOCKS = EPISODE_MAX_INODES;
	setup_tables();
